  ...
  return (
    <div className=" " >
      <Stepper
        ...
        className="my-6"
        style={{minWidth:"380px",overflowX:"auto"}}
      >
        {cardData.map((item) => (
          <Step key={item.name}>
            <StepLabel StepIconComponent={CustomStepIcon}>
              <p className="font-sansation text-custom-green">{item.name}</p>
              <p className="font-sansation text-sm">{item?.description ? item?.description : "-"}</p>
            </StepLabel>
          </Step>
        ))}
      </Stepper>
...
export default CheckoutStepper2;
